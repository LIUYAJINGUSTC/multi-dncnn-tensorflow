...
parser.add_argument('--sigma', dest='sigma', type=int, default=50, help='noise level')
...
def visualize():
	...
	numBatch = int(data.shape[0] / 128)
	print("[*] Data shape = " + str(data.shape))
	print("[*] Start training : ")
	for epoch in xrange(1):
		for batch_id in xrange(10):
			batch_images = data[batch_id:batch_id+1, :, :, :]
			train_images = add_noise(batch_images, 25 ,sess)
				
			#data2, label2= sess.run([batch_images, train_images])
			plt.figure()
			data_ = batch_images[0,:,:,0]
			plt.imshow(data_, cmap ='gray')

#		(Image.fromarray(np.uint8(label[0] * 255))).show()
			plt.figure()
			label_ = train_images[0,:,:,0]
