...
parser.add_argument('--src_dir', dest='src_dir', default='./BSD400', help='dir of data')
...
#			img_s = img # do not change the original img
	...
	for i in xrange(len(filepaths)):
		...
		for s in xrange(len(scales)):
			...
			for j in xrange(DATA_AUG_TIMES):
				...
				for x in range(0 + args.step, im_h - args.pat_size + 2, args.stride):
					for y in range(0 + args.step, im_w - args.pat_size + 2, args.stride):
						inputs[:, :, :, count] = data_augmentation(img_s[x:x + args.pat_size, y:y + args.pat_size, :],#j) 
																	random.randint(0, 7))
						count += 1
	# pad the batch
	if count < numPatches:
...
#	indexs = range(numPatches)
#	np.random.shuffle(indexs)
#	count=0
#	inputs2=np.zeros((args.pat_size,args.pat_size,1,numPatches))
#	for i in indexs:
